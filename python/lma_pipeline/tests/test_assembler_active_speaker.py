...
def result(result_id, items, is_partial=True, channel="AGENT"):
    return Result(result_id=result_id, channel=channel, is_partial=is_partial, items=items)
...
def test_unlabelled_items_bin_against_declared_active_speaker():
    ...
    out = asm.on_result(
        result("ra", [word("checking", 5.0, 5.3), word("your", 5.5, 5.7), word("mic", 5.9, 6.1)])
    )
    assert len(out) == 1
...
def test_mid_utterance_speaker_change_starts_new_segment_not_mutation():
    ...
    second = asm.on_result(
        result("rb", [word("thanks", 9.0, 9.3), word("bea", 9.5, 9.7), word("here", 9.9, 10.1)])
    )
    assert len(second) == 1
    ...
    closing = asm.on_result(
        result(
            "rc",
            [
                word("checking", 5.0, 5.3),
                word("your", 5.5, 5.7),
                word("mic", 5.9, 6.1),
                word("again", 6.2, 6.5),
            ],
...
def test_same_speaker_across_results_extends_one_segment():
    ...
    out = asm.on_result(result("rd", [word("thanks", 12.0, 12.3), word("again", 12.5, 12.7)]))
    assert len(out) == 1
...
def test_channel_without_any_active_speaker_declaration_stays_diarized_default():
    asm = SegmentAssembler("call-3")
    out = asm.on_result(result("re", [word("hello", 0.5, 0.8)], channel="CALLER"))
