...
WINDOW1 = [
    ("what", 21.0, 21.2), ("did", 21.3, 21.5),
    ("you", 21.6, 21.8), ("need", 21.9, 22.1),
    ("well", 22.5, 22.65),
    ("status", 23.0, 23.2), ("update", 23.3, 23.5),
    ("please", 23.6, 23.8), ("now", 23.9, 24.1),
    ("sure", 24.5, 24.7),
    ("here", 25.0, 25.2), ("it", 25.3, 25.5),
    ("is", 25.6, 25.8), ("then", 25.9, 26.2),
]

WINDOW1_LABELS = ["spk_0", "spk_0", "spk_0", "spk_0", "spk_1",
                  "spk_1", "spk_1", "spk_1", "spk_1", "spk_0",
                  "spk_0", "spk_0", "spk_0", "spk_0"]


def labelled_window1():
    return [word(content, start, end, label)
...
def test_every_diarized_id_matches_the_grammar():
    ...
    out = asm.on_result(Result(
        result_id="r4", channel="AGENT", is_partial=False,
        items=[word("aa", 0.0, 0.3, "spk_0"), word("bb", 0.5, 0.8, "spk_0"),
               word("cc", 25.0, 25.3, "spk_1"), word("dd", 25.5, 25.8, "spk_1")],
    ))
...
def test_r7_worked_example_partial_to_final_stability():
    ...
    p1 = asm.on_result(Result(result_id="r7", channel="CALLER",
    ...
    p2 = asm.on_result(Result(result_id="r7", channel="CALLER",
                              is_partial=True,
                              items=opening + [word(c, s, e)
                                               for c, s, e in WINDOW1]))
    ...
    final = asm.on_result(Result(
        ...
        items=[word(content, start, end, "spk_0")
               for content, start, end in OPENING] + labelled_window1(),
    ))
    assert [e["SegmentId"] for e in final] == [
        "r7-CALLER-w0-r0", "r7-CALLER-w1-r0",
        "r7-CALLER-w1-r1", "r7-CALLER-w1-r2",
    ]
