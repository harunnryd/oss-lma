...
def turn_a(labelled):
    ...
    items.append(
        WordItem(content=".", type="punctuation", start_time=13.5, end_time=13.5, speaker=None)
    )
    return items
...
def period_b():
    return WordItem(content=".", type="punctuation", start_time=35.0, end_time=35.0, speaker=None)
...
def test_thirty_five_second_two_turn_conversation_characterization():
    ...
    p1 = asm.on_result(
        Result(result_id="rA", channel="CALLER", is_partial=True, items=turn_a(False)[:7])
    )
    assert p1 == [
        event("rA-CALLER-w0-r0", None, 0.0, 6.5, "can you send me the updated budget", True)
    ]
    ...
    assert p2 == [
        event(
            ...
            "tomorrow morning please. sure i will export the",
            False,
        ),
    ...
    assert p3 == [
        event(
            ...
            20.5,
            33.0,
            "sheet tonight and share it in the channel right after dinner with the",
            True,
        )
    ]
    ...
    assert final == [
        ...
        event(
            ...
            20.5,
            35.0,
            "sheet tonight and share it in the channel right after dinner with the whole team.",
            False,
        ),
    ]
